...
ActiveRecord::Schema[8.1].define(version: 2026_04_25_180000) do
  ...
  create_table "nodes", id: :uuid, default: -> { "gen_random_uuid()" }, force: :cascade do |t|
    ...
    t.index ["node_type", "content"], name: "index_nodes_on_node_type_and_content_for_named_types", unique: true, where: "((node_type)::text = ANY (ARRAY[('need'::character varying)::text, ('person'::character varying)::text]))"
    t.index ["node_type"], name: "index_nodes_on_node_type"
    t.index ["source_uris"], name: "index_nodes_on_source_uris", using: :gin
  end
